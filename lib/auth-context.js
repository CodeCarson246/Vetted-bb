...
import { forgetMyFreelancer } from './myFreelancer'
...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        // Signing out (from any button, or the session expiring) wipes what
        // this device kept for the account: the saved dashboard holds the
        // user's own messages and details.
        if (event === 'SIGNED_OUT') { clearAllDashboardCaches(); forgetMyFreelancer() }
