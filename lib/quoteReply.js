...
// Receipt replies are announced as "Sent receipt for <ref> — paid in full on
// <date>. Total $<amount>." Detect and parse that line so a receipt can render
// as a card even when its linked quote can't be resolved (legacy rows that
// predate quote_id on receipt replies).
export function isReceiptBody(body) {
  return typeof body === 'string' && body.startsWith('Sent receipt')
}

// Accepts both separators: new rows use " - ", rows written before the
// em-dash cleanup used " — ". Both must keep parsing.
const RECEIPT_LINE = /^Sent receipt for (.+?) (?:—|-) paid in full on (.+?)\.\s*Total \$([\d,]+(?:\.\d+)?)/

export function parseReceiptLine(body) {
  if (typeof body !== 'string') return null
  const m = body.match(RECEIPT_LINE)
  if (!m) return isReceiptBody(body) ? { ref: null, paidOn: null, total: null } : null
  return { ref: m[1], paidOn: m[2], total: m[3] }
}
