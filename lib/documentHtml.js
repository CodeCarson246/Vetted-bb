...
import { termDays, termLabel } from './paymentTerms.js'
 ...
 * @param {object} [opts]  { type: 'quote' | 'invoice' | 'receipt', forPdf } —
 *                         forPdf leaves out the auto-print script: the server
 *                         renderer (lib/renderPdf.js) fits the page itself.
...
export function buildDocumentHtml(quote, issuer, opts = {}) {
  ...
  // Quotes show payment terms, not a due date: nothing is owed until the
  // work is invoiced, and a date on a quote reads like a demand for payment.
  const quoteTerms = quote.payment_terms
    ? (termDays(quote.payment_terms) === 0 ? 'Due on receipt of invoice' : `${termLabel(quote.payment_terms)} from invoice date`)
    : 'Set when invoiced'
  ...
  const clientPhone = formatPhone(quote.client_phone)
  ...
  const phone = issuer?.phone_verified ? formatPhone(issuer?.phone) : ''
      ...
      <td width="40%">
        <table width="100%">
          <tr>
            ...
            <td style="padding:10px 14px 10px 0;border-top:2px solid #111827;font-size:14px;font-weight:700;color:#00267F;text-align:right">${sym}${total}</td>
          </tr>
        </table>
      </td>
    </tr>
  ...
  <table width="100%" class="keep" style="margin-bottom:12px">
    <tr>
      <td style="background:${isReceipt ? '#DCFCE7' : '#EEF2FF'};border-radius:10px;padding:16px 18px">
        <div style="font-size:12px;font-weight:600;color:#374151;margin-bottom:4px">${isReceipt ? 'Paid in full' : isInvoice ? 'Payment due' : 'Payment terms'}</div>
        <div style="font-size:16px;font-weight:700;color:${isReceipt ? '#166534' : '#00267F'};margin-bottom:3px">${isReceipt ? (paidDate ? `Received ${docDateFmt(paidDate)}` : 'Received') : isInvoice ? docDateFmt(dueDate) : quoteTerms}</div>
        ${isReceipt ? `<div style="font-size:12px;color:#15803d">Thank you. This payment has been received in full.</div>` : ''}
        ${!isInvoice ? `<div style="font-size:12px;color:#6b7280">This is a quote, not a request for payment. An invoice follows once the work is agreed.</div>` : ''}
      </td>
    </tr>
  </table>
  ${[
