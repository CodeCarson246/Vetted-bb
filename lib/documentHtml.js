...
<style>
  * { box-sizing:border-box; margin:0; padding:0; -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
  ...
  /* No body padding in print: the page margin is the only margin (padding on
     ...
     that page so it prints on one sheet. */
  @page { margin:1.2cm; }
  @media screen { body { padding:40px; } }
  @media print { body { padding:0; zoom:var(--fit, 1); } }
...
<body>
  ...
  <script>
    (function () {
      ...
      // little, zoom it down for print so it lands on one sheet; if it is
      // genuinely long, leave it at full size and let it paginate. Zoom
      // widens the layout, which only shortens it, so the result still fits.
      function fit () {
        var W = 650, H = 912, MIN = 0.85, SAFETY = 0.98
        var b = document.body, st = b.style, pw = st.width, pp = st.padding
        st.padding = '0'; st.width = W + 'px'
        var h = b.scrollHeight
        st.width = pw; st.padding = pp
        if (h <= H) return
        var z = (H / h) * SAFETY
        if (z >= MIN) document.documentElement.style.setProperty('--fit', z.toFixed(3))
      }
      var done = false
