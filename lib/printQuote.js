...
import { currencySymbol } from './organisations'
import { formatVerifyCode } from './verifyCode'
import { SITE_HOST } from './siteUrl'
...
export function printSavedQuote(quote, issuer, opts = {}) {
  ...
  const itemRows = (quote.items || []).map((item, i) => `
    <tr>
      ...
      <td style="padding:10px 14px;font-size:13px;color:#374151;text-align:right;border-bottom:1px solid #f3f4f6;background:${i % 2 === 0 ? '#ffffff' : '#f9fafb'}">${item.price ? sym +parseFloat(item.price).toFixed(2) : ''}</td>
      <td style="padding:10px 14px;font-size:13px;font-weight:600;color:#111827;text-align:right;border-bottom:1px solid #f3f4f6;background:${i % 2 === 0 ? '#ffffff' : '#f9fafb'}">${item.price ? sym +((parseFloat(item.price) || 0) * (parseInt(item.qty) || 1)).toFixed(2) : ''}</td>
    </tr>`).join('')
...
<style>
  * { box-sizing:border-box; margin:0; padding:0; -webkit-print-color-adjust:exact !important; print-color-adjust:exact !important; }
  body { font-family:'Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif; background:white; color:#111827; padding:40px; position:relative; font-size:13px; line-height:1.45; }
...
<body>
  ...
  <table width="100%" style="margin-bottom:22px">
    <tr>
      <td style="vertical-align:middle">
        <span class="sora" style="font-size:18px;font-weight:800;letter-spacing:-0.5px;color:#00267F">Vetted<span style="color:#F9C000">.</span>bb</span>
      </td>
      <td style="vertical-align:middle;text-align:right;font-size:10px;color:#9ca3af;letter-spacing:0.08em;text-transform:uppercase">
        ${isReceipt ? 'Payment receipt' : isInvoice ? 'Invoice' : 'Quotation'} &middot; ${esc(docNumber)}
      </td>
    </tr>
  </table>
        ...
        <table>
          <tr>
            ...
            <td style="vertical-align:top">
              ...
              ${quote.from_address ? `<div style="margin-top:6px">${addressLines(quote.from_address)}</div>` : ''}
            </td>
          </tr>
        </table>
      </td>
      ...
      <td width="40%">
        <table width="100%">
          <tr>
            ...
            <td style="padding:8px 0;border-top:1px solid #e5e7eb;font-size:13px;color:#111827;text-align:right">${sym}${total}</td>
          </tr>
          <tr>
  ...
  <script>
    (function () {
      var done = false
      function go () { if (done) return; done = true; setTimeout(function () { window.print() }, 150) }
