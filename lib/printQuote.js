...
/**
 * Print/download a saved quote as a PDF (via the browser print dialog).
 * One implementation shared by the inbox, client messages, and the
 * quotes dashboard — all user-supplied fields are HTML-escaped.
 *
 ...
 */
export function printSavedQuote(quote, issuer) {
...
<head>
<meta charset="utf-8"/>
<title>Quote-${esc(quote.quote_number)}-${esc(quote.client_name || '')}</title>
<style>
  ...
  <table width="100%" style="margin-bottom:28px">
    <tr>
      ...
      <td style="vertical-align:top;text-align:right;width:50%">
        ...
        <div style="font-size:12px;color:#9ca3af;margin-top:6px">${esc(quote.quote_number)}</div>
        <div style="font-size:12px;color:#9ca3af;margin-top:2px">${quoteDate(quote.quote_date)}</div>
      </td>
    </tr>
  </table>
  ...
  <table width="100%" style="margin-bottom:24px">
    <tr>
      <td style="background:#EEF2FF;border-radius:10px;padding:16px 18px">
        ...
        <div style="font-size:16px;font-weight:700;color:#00267F;margin-bottom:3px">${quoteDate(quote.due_date)}</div>
      </td>
    </tr>
  </table>
  ${quote.notes?.trim() ? `
