...
// Normalise a date-only string ('2026-06-11') to local noon so the
// printed date doesn't flip a day across timezones.
function quoteDate(value) {
  if (!value) return ''
  const normalized = /^\d{4}-\d{2}-\d{2}$/.test(value) ? `${value}T12:00:00` : value
  return new Date(normalized).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
}
 ...
 * @param {object} [opts]  { type: 'quote' | 'invoice' } — invoice mode
 *                         uses the invoice number, issue date and due
 *                         date set when the invoice was sent.
 */
export function printSavedQuote(quote, issuer, opts = {}) {
  const isInvoice = opts.type === 'invoice'
...
<head>
<meta charset="utf-8"/>
<title>${isInvoice ? 'Invoice' : 'Quote'}-${esc(docNumber)}-${esc(quote.client_name || '')}</title>
<style>
...
<body>
  ...
  <table width="100%" style="margin-bottom:24px">
    <tr>
      <td style="background:#EEF2FF;border-radius:10px;padding:16px 18px">
        ...
        <div style="font-size:16px;font-weight:700;color:#00267F;margin-bottom:3px">${quoteDate(dueDate)}</div>
      </td>
    </tr>
  </table>
  ${quote.notes?.trim() ? `
  ...
  // Open the document in a new tab and print there. iOS Safari silently
  // ignores print() from hidden iframes, so a real window is the only
  // approach that works on every device — desktop users get the print
  // dialog, phone users get the native print/save-as-PDF sheet.
  ...
  if (win) {
    ...
    win.focus()
    setTimeout(() => {
      try { win.print() } catch { /* user can print from the tab */ }
    }, 500)
    return
  }
