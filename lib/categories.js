...
/**
 * Every category a freelancer should surface under: their primary
 * `category` plus any `extra_categories`. Single source of truth shared by
 * search, the category pages, homepage counts, saved-search alerts and the
 * public profile, so the matching logic can't drift between them.
 */
export function effectiveCategories(f) {
  return [...new Set([f?.category, ...(Array.isArray(f?.extra_categories) ? f.extra_categories : [])].filter(Boolean))]
}

/** Max categories one profile may appear under (primary + 2 extras). */
