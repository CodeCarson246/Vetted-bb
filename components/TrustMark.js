...
//
//   verified      navy circle, white tick   a professional whose identity we confirmed
//   organisation  gold circle, navy tick    an organisation we checked is genuine
//   vendor        gold shield in a navy pill a professional registered with Treasury
//   featured      navy star in a gold pill   a professional we are highlighting
// The emblems themselves live in badgeArt.js.
...
export default function TrustMark({ kind, size = 16, withLabel = false, label }) {
  ...
  if (kind === 'organisation') {
    return (
      <span title={`${meta.title}: ${meta.explain}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, verticalAlign: 'middle', lineHeight: 0 }}>
        <BadgeGlyph kind="organisation" size={size} label={meta.title} />
  ...
  if (kind === 'vendor') {
    ...
    return (
      ...
      >
        <BadgeGlyph kind="vendor" size={Math.round(size * 0.8)} />
        {text}
      </span>
    )
  }
  ...
  return (
    ...
    >
      <BadgeGlyph kind="featured" size={12} style={{ marginRight: 4 }} />{text}
    </span>
  )
}
